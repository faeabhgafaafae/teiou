...
/**
 * 一時実行用マイグレーションスクリプト
 * migrations/add_course_to_results.sql を実行する。
...
$sql_path = __DIR__ . '/migrations/add_course_to_results.sql';
if (!file_exists($sql_path)) {
    http_response_code(404);
    echo json_encode(['error' => 'migrations/add_course_to_results.sql が見つかりません']);
    exit;
}
...
echo json_encode([
    ...
    'errors'  => $errors,
    'message' => $ok
        ? 'results テーブルに course カラムを追加しました'
        : 'マイグレーション実行中にエラーが発生しました（カラムが既に存在する場合もここに出ます）',
