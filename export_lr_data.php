...
// 最新period を player_id ごとに特定 (ROW_NUMBERなしの互換クエリ)
// year*10+period が最大のものが最新
...
echo "race_id,date,venue,lane," .
     ...
     "exhibit_time_raw,exhibit_time_rel,exhibit_time_rel_within," .
...
foreach ($entries_all as $e) {
    ...
    // exhibit_time 相対化: 0=最遅, 1=最速 (速いほど高スコア)
    ...
    if ($ex_raw !== null && isset($race_exhibit[$rid])) {
        ...
        // exhibit_time_rel_within: レース内最速からの差(秒)を符号反転 (速いほど0に近い)
        $ex_w = round((float)$ex_raw - $race_exhibit[$rid]['min'], 3);
    }
